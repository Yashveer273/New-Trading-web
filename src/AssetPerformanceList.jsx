...
const AssetPerformanceList = ({purchasesWithStock,onBuy,isProcessing}) => {
  ...
  // ✅ FILTER DIRECTLY
  const filteredAssets = purchasesWithStock.filter(item => item.status === activeTab);
  ...
  return (
    <div className="v-unit2">
      ...
      <div className="v-assets-body">
        ...
        {!isInitialLoading && filteredAssets.map((asset) => (
          <div key={asset.purchaseId} className="v-asset-card">
            ...
            <div className="card-content">
              ...
              <div className="v-header-row">
                <div className="v-asset-info">
                  ...
                  <div>
                    <h3 className="v-asset-name">Asset: {asset.stockName}</h3>
              ...
              <div className="v-grid">
                <div className="v-panel">
                  <div className="v-panel-header">
                    ...
                    <div className="v-badge-row">
                      <span className="v-mini-badge badge-blue">INITIAL: {asset.totalStockUnits}</span>
                      <span className="v-mini-badge badge-green">AVAILABLE: {asset.leftStockUnits}</span>
                    </div>
                  </div>
                  ...
                  <div className="v-data-grid">
                    <div>
                      ...
                      <p className="data-value">₹{asset.singleUnitValue.toFixed(2)}</p>
                    </div>
                    <div style={{ textAlign: 'right' }}>
                      <p className="data-label" style={{ justifyContent: 'flex-end' }}>Total Units</p>
                      <p className="data-value">{asset.totalStockUnits}</p>
                    </div>
                  </div>

                  <div style={{ marginTop: 16 }}>
                    <p className="data-label"><Landmark size={10} style={{ marginRight: 4 }} /> Sold Stock Amount</p>
                    <p className="data-value text-rose">₹{asset.soldStockAmount.toFixed(2)}</p>
                  </div>

                  <div className="v-data-grid v-divider">
                    <div>
                      <p className="data-label">Purchase Date</p>
                      <p style={{ fontSize: 10, color: '#9ca3af', margin: 0 }}>{new Date(asset.purchaseDate).toLocaleDateString('en-IN')}</p>
                    </div>
                    <div style={{ textAlign: 'right' }}>
                      <p className="data-label" style={{ justifyContent: 'flex-end' }}>Last Sold Date</p>
                      <p style={{ fontSize: 10, color: '#9ca3af', margin: 0 }}>{asset.lastSoldDate ? new Date(asset.lastSoldDate).toLocaleDateString('en-IN') : 'N/A'}</p>
                    </div>
                  </div>
                  ...
                  <div className="v-divider" style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                      ...
                      <span style={{ fontSize: 9, fontWeight: 900, color: '#22d3ee' }}>{asset.status}</span>
                    </div>
                    <div>
                      <span className="data-label">Purchase ID</span>
                      <span style={{ fontSize: 9, fontFamily: 'monospace', color: '#6b7280', wordBreak: 'break-all' }}>{asset.purchaseId}</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="v-panel">
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                    ...
                    <div style={{ textAlign: 'right' }}>
                      ...
                      <div className={asset.stockDetails.trend === 'UP' ? 'text-emerald' : 'text-rose'}>
                        {asset.stockDetails.trend === 'UP' ? <TrendingUp size={12} /> : <TrendingDown size={12} />}
                  ...
                  <div className="v-holding-box">
                    ...
                    <p className="value-xl">₹{(asset.stockDetails.currentPrice * selectedUnits).toFixed(2)}</p>
                  </div>
      ...
      `SELL Stocks (${selectedUnits})`
                  ...
                  <div className="v-footer-info">
                    ...
                    <span style={{ fontSize: 9 }}>
                      Last Updated: {new Date(asset.stockDetails.lastUpdated).toLocaleString('en-IN')}
                    </span>
                  </div>
                </div>
              </div>
